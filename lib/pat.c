...
static inline bool
is_total_key_size_over(grn_ctx *ctx, grn_pat *pat, uint64_t curr_total_key_size, uint32_t key_size)
{
  bool is_total_key_size_over = false;

  if (pat_is_key_large(pat)) {
    is_total_key_size_over =
      (curr_total_key_size < GRN_PAT_MAX_TOTAL_KEY_SIZE_LARGE &&
       key_size > GRN_PAT_MAX_TOTAL_KEY_SIZE_LARGE - curr_total_key_size);
  } else {
    is_total_key_size_over =
      (curr_total_key_size < GRN_PAT_MAX_TOTAL_KEY_SIZE &&
       key_size > GRN_PAT_MAX_TOTAL_KEY_SIZE - curr_total_key_size);
  }
  if (is_total_key_size_over) {
    GRN_DEFINE_NAME(pat);
...
key_put(grn_ctx *ctx, grn_pat *pat, const uint8_t *key, uint32_t len)
{
  ...
  is_total_key_size_over(ctx, pat, res, len);
  ...
  if (res >> W_OF_KEY_IN_A_SEGMENT != end_segment) {
     ...
     * segment. */
    pat_update_curr_key(ctx,
                        pat,
                        (uint64_t)(end_segment << W_OF_KEY_IN_A_SEGMENT));
